...
async def add_rooms(
    ...
    if not await db.hotels.get_one_or_none(id=room_data.hotel_id):
        raise HTTPException(
            status_code=409, detail="Отеля с таким hotel_id не существует"
        )
...
async def change_all_room_data(db: DBDep, room_id: int, room_data: RoomsAddRequest):
    _room_data = RoomsAdd(**room_data.model_dump())
    if not await db.hotels.get_one_or_none(id=room_data.hotel_id):
        raise HTTPException(
            status_code=409, detail="Отеля с таким hotel_id не существует"
...
async def change_partly_room_data(
    db: DBDep, room_id: int, room_data: RoomsPatchRequest
):
